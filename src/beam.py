...
class Beams(object):
    ...
    def infer_n(self):
        ...
        return semQL.NumA._init_grammar()
    ...
    def check_sel_valid(self, actions):
        ...
        sel_actions = list()
        for ac in actions:
            if type(ac) == semQL.Select:
                find_sel = True
            elif find_sel and type(ac) in [semQL.NumA, semQL.Table, semQL.Column, semQL.Agg]:
                if type(ac) not in [semQL.NumA]:
                    sel_actions.append(ac)
            elif find_sel and type(ac) not in [semQL.NumA, semQL.Table, semQL.Column, semQL.Agg]:
                break
