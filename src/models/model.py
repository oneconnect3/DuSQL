...
class IRNet(BasicModel):
    ...
    def __init__(self, args, grammar):
        ...
        self.src_type = nn.Linear(6, args.col_embed_size)
        self.col_type = nn.Linear(4, args.col_embed_size)
        self.table_type = nn.Linear(2, args.col_embed_size)
        self.value_type = nn.Linear(2, args.col_embed_size)
        ...
        self.N_embed = nn.Embedding(len(define_rule.NumA._init_grammar()), args.action_embed_size)
    ...
    def forward(self, examples):
        ...
        value_embedding = self.gen_x_batch(batch.value_name)
        ...
        for t in range(batch.max_sketch_num):
            if t == 0:
                ...
                for e_id, example in enumerate(examples):
                    if t < len(example.sketch):
                        ...
                        if type(action_tm1) in [define_rule.SQL,
                                                define_rule.SingleSQL,
                                                define_rule.Select,
                                                define_rule.Filter,
                                                define_rule.Superlative,
                                                define_rule.NumA,
                                                define_rule.Order
                                                ]:
                            a_tm1_embed = self.production_embed.weight[self.grammar.prod2id[action_tm1.production]]  #对每一个action进行embedding
                        elif type(action_tm1) in [define_rule.Value]:
                            # TODO value 部分损失函数的定义
            ...
            apply_rule_prob = F.softmax(self.production_readout(att_t), dim=-1)  # (bacth, 46)

            value_weights = self.value_pointer_net(src_encodings=value_embedding, query_vec=att_t.unsqueeze(0))
            value_weights.data.masked_fill_(batch.value_token_mask, -float('inf'))
            value_weights = F.softmax(value_weights, dim=-1)

            for e_id, example in enumerate(examples):
                if t < len(example.sketch):
                    action_t = example.sketch[t]
                    if isinstance(action_t, define_rule.Value):
        ...
        # torch.Size([batch size])
        #print(sketch_prob_var)

        col_embedding = self.gen_x_batch(batch.col_names)
        src_embedding = self.gen_x_batch(batch.src_sents)
        table_embedding = self.gen_x_batch(batch.table_names)

        # get emb differ
        embedding_differ_c = self.embedding_cosine(src_embedding=src_embedding, table_embedding=col_embedding,
                                                 table_unk_mask=batch.table_unk_mask)

        embedding_differ_t = self.embedding_cosine(src_embedding=src_embedding, table_embedding=table_embedding,
                                              table_unk_mask=batch.schema_token_mask)


        col_ctx = (src_encodings.unsqueeze(1) * embedding_differ_c.unsqueeze(3)).sum(2)
        table_ctx = (src_encodings.unsqueeze(1) * embedding_differ_t.unsqueeze(3)).sum(2)

        col_embedding = col_embedding + col_ctx
        table_embedding = table_embedding + table_ctx
        ...
        for t in range(batch.max_action_num):
                ...
                for e_id, example in enumerate(examples):
                    if t < len(example.tgt_actions):
                        ...
                        if type(action_tm1) in [define_rule.SQL,
                            ...
                            if isinstance(action_tm1, define_rule.Column):
                                a_tm1_embed = self.column_rnn_input(col_embedding[e_id, action_tm1.id_c])

                            elif isinstance(action_tm1, define_rule.Table):
                                a_tm1_embed = self.table_rnn_input(table_embedding[e_id, action_tm1.id_c])

                            elif isinstance(action_tm1, define_rule.Agg) or isinstance(action_tm1, define_rule.MathAgg):
                                a_tm1_embed = self.production_embed.weight[self.grammar.prod2id[action_tm1.production]]
                            else:
            ...
            if self.use_column_pointer:
                # att_t.shape (4,200)
                # 这为什么要乘(table_appear_mask_val)  和 (1 - table_appear_mask_val)
                gate = F.sigmoid(self.prob_att(att_t))
                weights = self.column_pointer_net(src_encodings=col_embedding, query_vec=att_t.unsqueeze(0),
                                                  src_token_mask=None) * table_appear_mask_val * gate + self.column_pointer_net(
                    src_encodings=col_embedding, query_vec=att_t.unsqueeze(0),
            ...
            for e_id, example in enumerate(examples):
                if t < len(example.tgt_actions):
                    ...
                    if isinstance(action_t, define_rule.Column):
                        ...
                        action_probs[e_id].append(act_prob_t_i)
                    elif isinstance(action_t, define_rule.Table):
                        act_prob_t_i = table_weights[e_id, action_t.id_c]
                        action_probs[e_id].append(act_prob_t_i)
                    elif isinstance(action_t, define_rule.Agg):
                        act_prob_t_i = apply_rule_prob[e_id, self.grammar.prod2id[action_t.production]]
                        action_probs[e_id].append(act_prob_t_i)
                    else:
                        pass
    ...
    def parse(self, examples, beam_size=5):
        ...
        src_encodings, (last_state, last_cell) = self.encode(batch.src_sents, batch.src_sents_len, None)
        src_encodings = self.dropout(src_encodings)             #为啥测试还加dropout
        # (1, seq_len, embed_dim)
        utterance_encodings_sketch_linear = self.att_sketch_linear(src_encodings)
        utterance_encodings_lf_linear = self.att_lf_linear(src_encodings)

        value_embedding = self.gen_x_batch(batch.value_name)
        value_type = self.input_type(batch.feature_v)
        value_type_var = self.value_type(value_type)
        value_embedding = value_embedding + value_type_var
        ...
        while len(completed_beams) < beam_size and t < self.args.decode_max_time_step:
            ...
            for hyp_id, hyp in enumerate(beams):
                ...
                if action_class in [define_rule.SQL,
                                    define_rule.SingleSQL,
                                    define_rule.Select,
                                    define_rule.Filter,
                                    define_rule.Superlative,
                ...
                elif action_class in [define_rule.Value]:
                    #TODO Value怎么处理
                    pass
                else:
        ...
        table_embedding = self.gen_x_batch(batch.col_names)
        src_embedding = self.gen_x_batch(batch.src_sents)
        ...
        col_type = self.input_type(batch.feature_c)
        ...
        while len(completed_beams) < beam_size and t < self.args.decode_max_time_step:
            ...
            for e_id, hyp in enumerate(beams):
                ...
                for act in hyp.actions:
                    if type(act) == define_rule.Column:
            ...
            if t == 0:
                ...
                for e_id, hyp in enumerate(beams):
                    ...
                    if type(action_tm1) in [define_rule.SQL,
                        ...
                        a_tm1_embed = self.production_embed.weight[self.grammar.prod2id[action_tm1.production]]
                        hyp.sketch_step += 1
                    elif isinstance(action_tm1, define_rule.Column):
                    ...
                    elif isinstance(action_tm1, define_rule.Agg):
                        a_tm1_embed = self.production_embed.weight[self.grammar.prod2id[action_tm1.production]]
                    else:
                        raise ValueError('unknown action %s' % action_tm1)
            ...
            for hyp_id, hyp in enumerate(beams):
                ...
                if type(padding_sketch[t]) == define_rule.Agg:
                    possible_productions = self.grammar.get_production(define_rule.Agg)
                    for possible_production in possible_productions:
                        prod_id = self.grammar.prod2id[possible_production]
                        prod_score = apply_rule_log_prob[hyp_id, prod_id]

                        new_hyp_score = hyp.score + prod_score.data.cpu()
                        meta_entry = {'action_type': define_rule.Agg, 'prod_id': prod_id,
                                      'score': prod_score, 'new_hyp_score': new_hyp_score,
                                      'prev_hyp_id': hyp_id}
                        new_hyp_meta.append(meta_entry)

                elif type(padding_sketch[t]) == define_rule.Column:
                    for col_id, _ in enumerate(batch.col_names[0]):
                        col_sel_score = column_selection_log_prob[hyp_id, col_id]
                        new_hyp_score = hyp.score + col_sel_score.data.cpu()
                        meta_entry = {'action_type': define_rule.Column, 'col_id': col_id,
                                      'score': col_sel_score, 'new_hyp_score': new_hyp_score,
                                      'prev_hyp_id': hyp_id}
                        new_hyp_meta.append(meta_entry)
                elif type(padding_sketch[t]) == define_rule.Table:
                    for t_id, _ in enumerate(batch.table_names[0]):
                        ...
                        meta_entry = {'action_type': define_rule.Table, 't_id': t_id,
            ...
            for new_hyp_score, meta_id in zip(top_new_hyp_scores.data.cpu(), meta_ids.data.cpu()):
                ...
                if 'prod_id' in hyp_meta_entry:
                    prod_id = hyp_meta_entry['prod_id']
                if action_type_str == define_rule.Column:
                    col_id = hyp_meta_entry['col_id']
                    action = define_rule.Column(col_id)
                elif action_type_str == define_rule.Table:
                    t_id = hyp_meta_entry['t_id']
                    action = define_rule.Table(t_id)
                elif action_type_str == define_rule.MathAgg:
                    t_id = hyp_meta_entry['t_id']
                    action = define_rule.MathAgg(t_id)
