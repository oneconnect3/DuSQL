...
class BasicModel(nn.Module):
    ...
    def padding_sketch(self, sketch):
        ...
        for action in sketch:
            ...
            if type(action) == define_rule.NumA:
                for _ in range(action.id_c + 1):
                    padding_result.append(define_rule.Agg(0))
                    padding_result.append(define_rule.Column(0))
                    padding_result.append(define_rule.Table(0))
            elif type(action) == define_rule.Filter and 'Agg' in action.production:
                padding_result.append(define_rule.Agg(0))
                padding_result.append(define_rule.Column(0))
                padding_result.append(define_rule.Table(0))
            elif type(action) == define_rule.Order or type(action) == define_rule.Superlative:
                padding_result.append(define_rule.Agg(0))
                padding_result.append(define_rule.Column(0))
                padding_result.append(define_rule.Table(0))
        ...
        for i, one_q in enumerate(q):
            if not is_list:
                ...
                for ws in one_q:
                    ...
                    ws_len = len(ws)
                    for w in ws:
                        emb_list.append(self.word_emb.get(w, self.word_emb['，']))
                    if ws_len == 0:
