...
from src.rule.semQL import Superlative, Select, Order, SingleSQL, Filter, Agg, NumA, Column, Table, SQL
...
def _build_single_filter(lf, f):
    ...
    if len(lf) == 0:
        ...
        table = lf.pop(0)
        if not isinstance(table, define_rule.Table):
            lf.insert(0, table)
            table = None
    assert isinstance(agg, define_rule.Agg) and isinstance(column, define_rule.Column)
...
def _build(lf):
    root = lf.pop(0)
    assert isinstance(root, define_rule.SingleSQL)
    length = len(root.production.split()) - 1
    while len(root.children) != length:
        c_instance = lf.pop(0)
        if isinstance(c_instance, define_rule.Select):
            ...
            assert isinstance(c_instance, define_rule.NumA)
        ...
        elif isinstance(c_instance, define_rule.Superlative) or isinstance(c_instance, define_rule.Order):
...
def build_tree(lf):
    root = lf.pop(0)
    assert isinstance(root, define_rule.SQL)
    if root.id_c == 0 or root.id_c == 1 or root.id_c == 2 or root.id_c == 3\
            or root.id_c == 4 or root.id_c == 5 or root.id_c == 6:
        root_1 = _build(lf)
        root_2 = _build(lf)
...
def verify(node):
    if isinstance(node, Column) and len(node.children) > 0:
    ...
    elif isinstance(node, SingleSQL):
        assert children_num == len(node.production.split()) - 1
    elif isinstance(node, NumA):
        assert children_num == int(node.id_c) + 1
    elif isinstance(node, Superlative) or isinstance(node, Order) or isinstance(node, Select):
        assert children_num == 1
...
def build_adjacency_matrix(lf, symmetry=False):
    _lf = list()
    for rule in lf:
        if isinstance(rule, Agg) or isinstance(rule, Column) or isinstance(rule, Table):
            continue
        _lf.append(rule)
    length = len(_lf)
