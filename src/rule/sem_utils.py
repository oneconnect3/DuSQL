...
def alter_not_in(args,datas, schemas):
    if args.v2:
        for d in datas:
            d['model_result'] = []
            for one_pre in d['all_preds']:
                if 'Filter(19)' in one_pre:
                    ...
                    origin_table_names = [[wordnet_lemmatizer.lemmatize(x.lower()) for x in names.split(' ')] for names
                                          in
                                          d['table_names']]
                    question_arg_type = d['question_arg_type']
                    question_arg = d['question_arg']
                    pred_label = one_pre.split(' ')
                        ...
                        if label_val in ['Filter(19)']:
                            ...
                            for pair in current_table['foreign_keys']:
                                if int(current_table['col_table'][pair[0]]) == h_table and d['col_set'].index(
                                        current_table['schema_content_clean'][pair[1]]) == int(
                                    pred_label[label_id + 2][2:-1]):
                                    ...
                                    break
                                        ...
                                        current_table['schema_content_clean'][pair[0]]) == int(
                                    pred_label[label_id + 2][2:-1]):
                                    pred_label[label_id + 8] = 'C(' + str(
                    ...
                    d['model_result'].append(" ".join(pred_label))
    else:
        for d in datas:
...
def alter_inter(args,datas):
    if args.v2:
        for d in datas:
            for one_pre in d['all_preds']:
                if 'Filter(0)' in one_pre:
                    now_result = one_pre.split(' ')
                    ...
                    d['model_result'].append(replace_result)
    else:
        for d in datas:
...
def alter_column0(args,datas):
    ...
    if args.v2:
        ...
        result = []
        for d in datas:
            d['model_result_replace'] = []
            for one_pre in d['all_preds']:
                if 'C(0)' in one_pre:
                    pattern = regex.compile('C\(.*?\) T\(.*?\)')
                    result_pattern = list(set(pattern.findall(one_pre)))
                    ground_col_labels = []
        ...
        for data in datas:
            data['model_result_replace'] = []
            for one_pre in d['all_preds']:
                if 'rule_count' in data:
                    str_replace = 'C(0) T(' + str(data['rule_count']) + ')'
                    replace_result = regex.sub('C\(0\) T\(.\)', str_replace, one_pre)
                    data['model_result_replace'].append(replace_result)
                else:
                    data['model_result_replace'].append(one_pre)
